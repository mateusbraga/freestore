	...
	for {
		...
		if receivedValue.Err != nil {
			if oldViewError, ok := receivedValue.Err.(*view.OldViewError); ok {
				log.Printf("View updated during read quorum: old '%v %v' new '%v %v'\n", thisClient.View(), thisClient.ViewRef(), oldViewError.NewView, view.ViewToViewRef(oldViewError.NewView))
